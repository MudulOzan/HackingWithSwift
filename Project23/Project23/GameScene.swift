...
//  Project23
//
//  Created by Ozan Mudul on 5.03.2023.
//
...
class GameScene: SKScene {
	...
	func createScore() {
		...
		gameScore.position = CGPointMake(8, 8)
		score = 0
	}
	...
	func createLives() {
		for i in 0..<3 {
			...
			spriteNode.position = CGPointMake(CGFloat(834 + (i * 70)), 720)
			addChild(spriteNode)
	...
	func createEnemy(forceBomb: ForceBomb = .random, isBonus: Bool = false) {
		...
		if enemyType == alwaysBomb {
			...
			if let emitter = SKEmitterNode(fileNamed: "sliceFuse") {
				emitter.position = CGPointMake(76, 64)
	...
	func endGame(triggeredByBomb: Bool) {
		...
		gameOver.position = CGPointMake(512, 334)
	...
	override func update(_ currentTime: TimeInterval) {
		...
		if activeEnemies.count > 0 {
			...
			if !nextSquenceQueued {
				DispatchQueue.main.asyncAfter(deadline: .now() + popupTime) { [weak self] in
					self?.tossEnemies()
				}
				
				nextSquenceQueued = true
			}
		}
